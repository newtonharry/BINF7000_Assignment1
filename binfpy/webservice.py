...
def search(query, dbName='uniprot', format='list', limit=100, columns=""):
    ...
    if dbName.startswith('uniprot'):
        ...
        if limit == None:  # no limit to number of results returned
            url = "{}{}/search?&format={}&query={}&columns={}".format(__uniprotUrl__, dbName, format,
                                                               urllib.parse.quote(query),
                                                               columns)
        else:
            url = "{}{}/search?format={}&limit={}&query={}&columns={}".format(__uniprotUrl__, dbName, format, str(limit),
